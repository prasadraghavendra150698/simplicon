 ...
 * Requires: RESEND_API_KEY, ADMIN_EMAIL env vars (add in Vercel dashboard)
 */
...
<body style="margin:0;padding:0;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background-color:#f0fdf4;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.08);">
    <tr><td style="padding:32px 40px;background:linear-gradient(135deg,#0c4a6e 0%,#082f49 100%);text-align:center;">
    ...
    <tr><td style="padding:24px 40px;background:#f8fafc;font-size:12px;color:#64748b;text-align:center;">Simplicon Tax Advisors LLP | Trading as Simplicon Tax</td></tr>
  </table>
</body>
</html>`;
}
...
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Thank You - Simplicon Tax Advisors</title></head>
<body style="margin:0;padding:0;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background-color:#f0fdf4;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.08);">
    <tr><td style="padding:32px 40px;background:linear-gradient(135deg,#0c4a6e 0%,#082f49 100%);text-align:center;">
      <img src="${logoUrl}" alt="Simplicon Tax Advisors" width="160" height="48" style="object-fit:contain;">
    ...
    <tr><td style="padding:40px;">
      ...
      <p style="margin:0 0 16px;font-size:16px;color:#475569;line-height:1.6;">We have received your message and will get back to you within 1-2 business days.</p>
      <p style="margin:0 0 24px;font-size:16px;color:#475569;line-height:1.6;">In the meantime, feel free to reach us at <a href="mailto:info@simplicontax.com" style="color:#0c4a6e;text-decoration:none;">info@simplicontax.com</a> or call <a href="tel:+919959778797" style="color:#0c4a6e;text-decoration:none;">+91 99597 78797</a>.</p>
      <p style="margin:0;font-size:16px;color:#475569;line-height:1.6;">Best regards,<br><strong style="color:#0f172a;">Simplicon Tax Advisors LLP</strong></p>
    </td></tr>
    <tr><td style="padding:24px 40px;background:#f8fafc;font-size:12px;color:#64748b;text-align:center;">Simplicon Tax Advisors LLP | Trading as Simplicon Tax | Founded in 2024</td></tr>
  </table>
</body>
</html>`;
}
...
export default async function handler(request: Request): Promise<Response> {
  ...
  const origin = request.headers.get('origin') || request.headers.get('referer')?.replace(/\/$/, '') || 'https://simplicon-tax.vercel.app';
  const logoUrl = `${origin}/Simplicon_logo.png`;
  ...
  if (!process.env.RESEND_API_KEY) {
    ...
    return new Response(
      JSON.stringify({ error: 'Email service not configured. Add RESEND_API_KEY in Vercel environment variables.' }),
  ...
  try {
    const { error: adminError } = await resend.emails.send({
      ...
      html: adminEmailHtml(
        { name: name.trim(), email: email.trim(), phone: (phone || '').trim(), subject: subject || 'general', message: message.trim() },
        logoUrl
      ),
    });
    ...
    if (adminError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to send form submission' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (ackError) console.error('Acknowledgement email error:', ackError);
    ...
    return new Response(JSON.stringify({ error: 'Failed to process form submission' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
